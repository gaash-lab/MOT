...
class Tracker(object):
    def __init__(self, args, vid_name):
        ...
        self.memory_bank = [] # Memory bank for storing past detections
    ...
    def match_with_memory(self, det):
        ...
        # Check memory bank for a match
        for mem in self.memory_bank:
            mem_h, mem_w = mem['hw']
            mem_area = mem['area']

            if abs(mem_h - h) < 10 and abs(mem_w - w) < 10 and abs(mem_area - area) < 200:
                matched_id = mem['id']
                self.memory_bank.remove(mem)  # Remove so it won't be reused again
                return matched_id
        ...
        for idx, flag in enumerate(allow_indices):
            if flag:
                ...
                if reused_id is not None:
                    det.initiate(self.frame_id, self.counter, reused_id)  # You'll add this method
                else:
